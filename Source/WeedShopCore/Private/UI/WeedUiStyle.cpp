...
#include "Cultivation/PotTypes.h"    // pot-def (quality cap / yield / plant-slots) voor BuildItemDetail
...
namespace WeedUI
{
	...
	// Type-tag + korte hint + accent-kleur op basis van het item-id-prefix. (1-op-1 uit HandInfoWidget's
	// ClassifyItem gehaald zodat de hand-preview en de inventory-quick-view exact dezelfde tag/kleur/hint delen.)
	static void ClassifyItem(const FString& Id, FString& OutType, FString& OutHint, FLinearColor& OutCol)
	{
	...
	FItemDetailInfo BuildItemDetail(UObject* WorldContext, FName Id, int32 Qty, float Thc, float QualPct)
	{
		FItemDetailInfo Info;
		const FString IdStr = Id.ToString();

		// Type-tag + hint + accent-kleur (gedeelde ClassifyItem).
		...
		// Winkel-catalogus voor de hint (concrete werking, bv. "+15% yield this harvest") en de strain-stats.
		UStoreComponent* Store = nullptr;
		const UWorld* World = GEngine ? GEngine->GetWorldFromContextObject(WorldContext, EGetWorldErrorMode::ReturnNull) : nullptr;
		if (const AWeedShopGameState* GS = World ? World->GetGameState<AWeedShopGameState>() : nullptr) { Store = GS->GetStore(); }

		// Winkel-omschrijving als hint (heeft vaak concrete getallen), behalve voor wiet/joints (niet uit de catalogus).
		...
		if (Store && !bWeedProduct)
		{
			...
			if (!Desc.IsEmpty()) { Info.Hint = Desc; }
		}

		// Stat-rijen per item-type (1-op-1 uit HandInfoWidget's stat-switch). Label -> waarde.
		auto AddStat = [&Info](const FString& Label, const FString& Value) { Info.Stats.Emplace(Label, Value); };
		...
		else if (IdStr.StartsWith(TEXT("Joint_")))
		{
			...
			if (!JStrain.IsNone()) { AddStat(TEXT("Strain"), PrettyItemName(FName(*(TEXT("Bud_") + JStrain.ToString())))); }
		...
		// Waterfles: GEEN "Fill X/Y"-stat -> de groene "3/3"-badge bij de titel zegt het al (dubbel = clutter).
