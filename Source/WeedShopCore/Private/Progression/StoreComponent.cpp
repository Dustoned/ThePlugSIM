...
namespace
{
	...
	static const FSupplyDef GSupplies[] = {
		...
		{ TEXT("Papers_Small"),     TEXT("Rolling papers"),   TEXT("Up to 2g per joint - 10 pcs"),  1000, 10 },
		{ TEXT("Papers_Big"),       TEXT("King-size papers"), TEXT("Up to 5g per joint - 10 pcs"), 3000, 10 },
		{ TEXT("Papers_Blunt"),     TEXT("Blunt wraps"),      TEXT("Up to 7g per joint - 10 pcs"), 9000, 10 },
		{ TEXT("Papers_Backwoods"), TEXT("Backwoods"),        TEXT("Up to 10g per joint - 5 pcs"), 20000, 5 },
		// Pots (betere pot = betere waterretentie/kwaliteit + meer yield).
		{ TEXT("Pot_Broken"),       TEXT("Broken pot"),  TEXT("Leaks - low quality, 1 plant"),    3000, 1 },
		...
		{ TEXT("Soil_Premium"),     TEXT("Premium soil"), TEXT("Top yield - 6 harvests"),         27000, 1 },
		// Water-flessen (betere fles = meer water, minder vaak vullen).
		{ TEXT("WaterBottle_Plastic"),  TEXT("Plastic bottle"), TEXT("3 waterings per fill"),  2000,  1 },
		{ TEXT("WaterBottle_Steel"),    TEXT("Steel bottle"),   TEXT("6 waterings per fill"),  9000,  1 },
		{ TEXT("WaterBottle_Jerrycan"), TEXT("Jerry can"),      TEXT("12 waterings per fill"), 45000,  1 },
		{ TEXT("WaterBottle_Tank"),     TEXT("Water tank"),     TEXT("25 waterings per fill"), 180000,  1 },
		// Droogrekken (vers geoogste wiet eerst drogen voor verkoop). Meer/sneller = duurder.
		{ TEXT("DryRack_Cheap"), TEXT("Cheap drying rack"),  TEXT("2 batches, slow (~3 min)"),    16000, 1 },
		{ TEXT("DryRack_Std"),   TEXT("Drying rack"),        TEXT("5 batches, faster (~2 min)"), 75000, 1 },
		{ TEXT("DryRack_Pro"),   TEXT("Pro drying cabinet"), TEXT("10 batches, fast (~1 min)"),  280000, 1 },
		// Verpak-tafel + bakjes/jars (verdeel gedroogde wiet in verkoopbare verpakkingen).
		{ TEXT("Bench_Pack"),  TEXT("Packing bench"),            TEXT("1 bag at a time"),  24000, 1 },
		{ TEXT("Bench_Pack2"), TEXT("Pro packing bench"),        TEXT("3 bags at a time"),  120000, 1 },
		{ TEXT("Bench_Pack3"), TEXT("Industrial packing table"), TEXT("6 bags at a time"), 550000, 1 },
		// Hasj-keten: Mesh-extractor (wiet -> crystals) + Heatpress (crystals -> hasj, hogere THC%).
		{ TEXT("Mesh_Cheap"),  TEXT("Mesh extractor"),       TEXT("Dried weed -> THC crystals (15%, ~60s)"),     27000, 1 },
		{ TEXT("Mesh_Std"),    TEXT("Pro mesh extractor"),   TEXT("More yield + faster (20%, ~45s, 2 batches)"), 112000, 1 },
		{ TEXT("Mesh_Pro"),    TEXT("Industrial extractor"), TEXT("Top yield (28%, ~30s, 3 batches)"),           300000, 1 },
		{ TEXT("Press_Cheap"), TEXT("Heatpress"),            TEXT("Crystals -> hash, higher THC (~90s)"),        54000, 1 },
		{ TEXT("Press_Std"),   TEXT("Pro heatpress"),        TEXT("Less loss + faster (~70s, 2 batches)"),       180000, 1 },
		{ TEXT("Press_Pro"),   TEXT("Industrial press"),     TEXT("Best yield + THC (~50s, 3 batches)"),        600000, 1 },
		...
		{ TEXT("Oven_Std"),    TEXT("Oven / stove"),         TEXT("Dried weed -> baked weed (decarb, ~40s)"),           24000, 1 },
		{ TEXT("Pan_Std"),     TEXT("Cooking pan"),          TEXT("Baked weed + butter -> cannabutter mix (~55s)"),     28000, 1 },
		{ TEXT("Butter"),      TEXT("Butter"),               TEXT("Cooking ingredient for the pan (cannabutter)"),        600, 1 },
		// Bak-ingredienten voor de koelkast-keuken: butter mix + deze -> cookies of gummies (eindproduct).
		{ TEXT("Flour"),       TEXT("Flour"),                TEXT("Baking ingredient: butter mix + flour + sugar -> cookies"),   400, 1 },
		{ TEXT("Sugar"),       TEXT("Sugar"),                TEXT("Baking ingredient for cookies and gummies"),                 300, 1 },
		{ TEXT("Gelatin"),     TEXT("Gelatin"),              TEXT("Setting ingredient: butter mix + gelatin + sugar -> gummies"), 500, 1 },
		// Pro-edibles: sneller + grotere capaciteit (zelfde THC). Voor massa-productie.
		{ TEXT("Oven_Pro"),    TEXT("Pro range cooker"),     TEXT("Faster bake + 4 batches (~25s)"),                    120000, 1 },
		{ TEXT("Pan_Pro"),     TEXT("Pro cooktop"),          TEXT("Faster cook + 4 batches, less loss (~35s)"),         144000, 1 },
		// Concentraat-ketens: solventless rosin, ice/bubble (isolator) + moonrocks. Eigen machines, sterke producten.
		{ TEXT("Rosin_Std"),   TEXT("Rosin press"),          TEXT("Dried weed -> solventless rosin (high THC, ~75s)"),      150000,  1 },
		{ TEXT("Rosin_Pro"),   TEXT("Hydraulic rosin press"),TEXT("More yield + faster (~55s, 2 batches)"),                 420000,  1 },
		{ TEXT("Iso_Std"),     TEXT("Ice extractor"),        TEXT("Dried weed -> bubble/ice hash (top THC, ~95s)"),         210000,  1 },
		{ TEXT("Iso_Pro"),     TEXT("Pro isolator"),         TEXT("More yield + faster (~70s, 2 batches)"),                 540000,  1 },
		{ TEXT("Oil_Std"),     TEXT("Oil press"),            TEXT("Dried weed -> cannabis oil (coating for moonrocks, ~80s)"), 150000, 1 },
		{ TEXT("Oil_Pro"),     TEXT("Pro oil press"),        TEXT("More oil + faster (~60s, 2 batches)"),                      375000, 1 },
		{ TEXT("Moon_Std"),    TEXT("Moonrock station"),     TEXT("Dried weed + oil -> moonrocks (coated bud, big yield, ~60s)"), 225000,  1 },
		{ TEXT("Moon_Pro"),    TEXT("Pro moonrock station"), TEXT("More yield + faster (~45s, 4 batches)"),                550000,  1 },
		// Losse upgrade-gear voor droogrek / hasj-machines (zet 'm vlakbij de machine).
		{ TEXT("DryUp_FanSmall"), TEXT("Small drying fan"), TEXT("Nearby drying rack: ~15% faster"), 9000, 1 },
		{ TEXT("DryUp_Fan"),    TEXT("Drying fan"),      TEXT("Nearby drying rack: ~30% faster"),        27000,  1 },
		...
		{ TEXT("Cont_Jar15"), TEXT("Big jars"),       TEXT("Jar up to 50g - 5 pcs"),   18000,  5 },
		{ TEXT("Cont_Block100"),  TEXT("Press blocks"), TEXT("Up to 100g - 3 pcs (bulk)"),   36000, 3 },
		{ TEXT("Cont_Garbage500"),TEXT("Garbage bags"), TEXT("Up to 500g - 2 pcs (bulk)"),  150000, 2 },
		...
		{ TEXT("Fridge"),   TEXT("Fridge"),   TEXT("Keeps food cold + sets butter mix into edibles (no conversion kit needed)"),    30000, 1 },
		{ TEXT("Shelf"),    TEXT("Storage shelf"), TEXT("Store stock in the shop - 24 slots"), 36000, 1 },
		{ TEXT("Chest"),    TEXT("Storage chest"), TEXT("Stash items at home - 20 slots"),     18000, 1 },
		// Kluizen (progressie): cash veilig stashen, een overval pakt 'm niet. Grotere kluis = meer capaciteit.
		{ TEXT("Safe_Small"),  TEXT("Small safe"),  TEXT("Stash up to EUR 10k - robbery-proof"),    800000,   1 },
		{ TEXT("Safe_Medium"), TEXT("Medium safe"), TEXT("Stash up to EUR 50k - robbery-proof"),    5400000,  1 },
		{ TEXT("Safe_Large"),  TEXT("Large safe"),  TEXT("Stash up to EUR 250k - robbery-proof"),   40000000,  1 },
		{ TEXT("Safe_Vault"),  TEXT("Vault"),       TEXT("Stash up to EUR 1M - robbery-proof"),     180000000, 1 },
		...
		{ TEXT("Fertilizer_Basic"), TEXT("Fertilizer"),       TEXT("+15% yield this harvest"),          6000, 3 },
		{ TEXT("Fertilizer_Bloom"), TEXT("Bloom fertilizer"), TEXT("+30% yield this harvest"),          18000, 3 },
		{ TEXT("Spray_Fungicide"),  TEXT("Fungicide spray"),  TEXT("Cures MOLD on a plant"),            7500, 5 },
		{ TEXT("Spray_Pesticide"),  TEXT("Pesticide spray"),  TEXT("Cures PESTS on a plant"),           7500, 5 },
		{ TEXT("Spray_Broad"),      TEXT("Broad-spectrum spray"), TEXT("Cures both mold AND pests"),    30000, 5 },
		// Pot-gear: fysieke accessoires. Koop -> plaats naast je pot -> bonus zolang 't er staat.
		{ TEXT("Gear_Drainage"),  TEXT("Drainage layer"), TEXT("+10% max quality (place by a pot)"),   6000, 1 },
		{ TEXT("Gear_Insulation"),TEXT("Insulation wrap"),TEXT("Dries out 2x slower"),                  8000, 1 },
		{ TEXT("Gear_Bloom"),     TEXT("Bloom booster"),  TEXT("+20% harvest yield"),                   10000, 1 },
		{ TEXT("Gear_Lamp1"),     TEXT("Grow lamp I"),    TEXT("+15% faster growth"),                   12000, 1 },
		{ TEXT("Gear_Lamp2"),     TEXT("Grow lamp II"),   TEXT("+30% faster growth"),                  36000, 1 },
		{ TEXT("Gear_Lamp3"),     TEXT("Grow lamp III"),  TEXT("+50% faster growth"),                  110000, 1 },
		{ TEXT("Gear_Tent1"),     TEXT("Grow tent I"),    TEXT("+8% max quality"),                      14000, 1 },
		{ TEXT("Gear_Tent2"),     TEXT("Grow tent II"),   TEXT("+15% max quality"),                    39000, 1 },
		{ TEXT("Gear_Tent3"),     TEXT("Grow tent III"),  TEXT("+22% max quality"),                    120000, 1 },
		{ TEXT("Gear_Water1"),    TEXT("Auto-water I"),   TEXT("Keeps the pot watered"),               42000, 1 },
		{ TEXT("Gear_Water2"),    TEXT("Auto-water II"),  TEXT("Watered + nutrients (+10% yield)"),     104000, 1 },
	};
}
